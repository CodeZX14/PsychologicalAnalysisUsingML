...
if os.path.isfile("label_list"):
    ...
    with open("feature_list", "r") as features, open("label_list", "a") as labels:
        for line in features:
            ...
            labels.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t" % (str(baseline_angle), str(top_margin), str(
                letter_size), str(line_spacing), str(word_spacing), str(pen_pressure), str(slant_angle)))
            labels.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t" % (str(trait_1), str(trait_2), str(
